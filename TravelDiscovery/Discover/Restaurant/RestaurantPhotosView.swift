...
struct RestaurantPhotosView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ScrollView {
                Spacer().fullScreenCover(isPresented: $isShowingFullScreenModal,content: {
                    ZStack(alignment: .topLeading) {
                        Color.black.ignoresSafeArea()
                        RestaurantCarouselView(imageUrlStrings: imageUrlStrings, selectedIndex: selectedPhotoIndex)
                        Button {
                            isShowingFullScreenModal.toggle()
                        } label: {
                ...
                .opacity(isShowingFullScreenModal ? 1 : 0)
                ...
                Picker("Test", selection: $mode) {
                    ...
                    .padding()
                if mode == "grid" {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: proxy.size.width / 3 - 6, maximum: 600),spacing: 2)],spacing: 4) {
                        ForEach(imageUrlStrings, id: \.self) { imageUrlString in
                            Button {
                                isShowingFullScreenModal.toggle()
                                self.selectedPhotoIndex = imageUrlStrings.firstIndex(of: imageUrlString) ?? 0
                            } label: {
                    ...
                    ForEach(imageUrlStrings,id: \.self) {imageUrlString in
                        ...
                        KFImage(URL(string: imageUrlString))
                            ...
                            HStack {
                                ...
                                .font(.system(size: 22))
                            Text("Description of text goes here make sure to put it there later \n \n Great job everyone")
                                .font(.system(size: 14))
                                .padding(.horizontal,8)
                            Text("Posted on 10/02/20")
                                .font(.system(size: 14))
                                .padding(.horizontal,8)
